...
def get_trainer(gpu, epochs):
    """ get the trainer, modify here it's options:
        - save_top_k
        - max_epochs
     """
    checkpoint_callback = ModelCheckpoint(monitor='val_loss_epoch', save_top_k=-1, 
                                          filename='{epoch:02d}-{val_loss_epoch:.6f}', dirpath='checkpoints/')
...
def train(gpu, region_id, mode, epochs, checkpoint_path):
    ...
    warmup_steps = total_training_steps // params['training_params']['warmup']
    params['data_params']['n_training_steps'] = total_training_steps
    params['data_params']['n_warmup_steps'] = warmup_steps
    params['data_params']['lr'] = params['training_params']['lr']
    params['data_params']['scheduler'] = params['training_params']['scheduler']
    ...
    # trainer
    # ------------
    trainer = get_trainer(gpu, epochs)
...
def set_parser():
    ...
    parser.add_argument("-e", "--epochs", type=int, required=False, default=1, 
                        help="number of epochs to train. 1 as default")
...
def main():
    ...
    train(options.gpu_id, options.region, options.mode, options.epochs, options.checkpoint)
