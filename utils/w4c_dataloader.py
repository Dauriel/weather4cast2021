...
class NWCSAF(Dataset):
    ...
    def __init__(self, data_split, products, target_vars,
                 ...
                 len_seq_out=4, region_id=None,  preprocess=None,
    ...
    def load_in_seq(self, day_id, in_start_id):
    ...
    def load_out_seq(self, day_id, in_start_id):
        """ load the input sequence """
        # 1. load nwcsaf products & metadata
        in_seq, in_info = data_utils.get_sequence_netcdf4(self.len_seq_out, in_start_id, day_id, 
                                                         self.products, self.data_path, self.target_vars, 
                                                         crop=self.crop_in, preprocess=self.preprocess['source'], 
                                                         day_bins=self.day_bins, 
                                                         sorted_dates=self.day_paths.id_date.sort_values().values)
    ...
    def load_in_out(self, day_id, in_start_id):
        ...
        # load input sequence
        in_seq, in_info = self.load_in_seq(day_id, in_start_id)
        # load ground truth
        if self.data_split != 'test':
            target_time = in_start_id + self.len_seq_in
            out_seq, out_info = self.load_out_seq(day_id, target_time)
            metadata = {'in': in_info, 
                        'out': out_info}
        else:
            out_seq = np.asarray([]) # we don't have the ground truth for the test split
            metadata = {'in': in_info, 
                        'out': {'day_in_year': [day_id]}}
