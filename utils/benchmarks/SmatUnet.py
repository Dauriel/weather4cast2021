...
from benchmarks.SmaAt_UNet import SmaAt_UNet
from benchmarks.optimizer import get_linear_schedule_with_warmup
...
class FeaturesSysUNet(pl.LightningModule):
    def __init__(self, UNet_params: dict, extra_data: str, depth: int, height: int, 
                 ...
                 seq_mode: str, lr: float, scheduler: str, n_training_steps: int, n_warmup_steps: int, **kwargs):
        super(FeaturesSysUNet, self).__init__()

        
        self.save_hyperparameters()
        self.model = SmaAt_UNet(**UNet_params)
    ...
    def configure_optimizers(self):
        ...
        if self.scheduler != '':  
            if self.scheduler == 'LW':
                scheduler = get_linear_schedule_with_warmup(
                              optimizer,
                              num_warmup_steps=self.n_warmup_steps,
                              num_training_steps=self.n_training_steps
                            )
            elif self.scheduler == 'CA':
                scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.n_warmup_steps)
            elif self.scheduler == 'CAR':
                scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=self.n_warmup_steps)
