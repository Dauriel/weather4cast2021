...
def main():
    ...
    r1 = ["/content/R1/" + x for x in list(os.listdir("/content/R1"))] 
    r2 = ["/content/R2/" + x for x in list(os.listdir("/content/R2"))] 
    r3 = ["/content/R3/" + x for x in list(os.listdir("/content/R3"))] 
    checkpoint_paths = {'R1': r1,'R2': r2, 'R3': r3}
    challenges = {'w4c-core-stage-1': ['R1', 'R2', 'R3'], 'w4c-transfer-learning-stage-1': ['R4', 'R5', 'R6']}
    ...
    for region in challenges[task_name]:
        ...
        models = [load_model(Model, params, path, device=device) for path in checkpoint_paths[region]]
